  ...
  u.extend(u.FE.DEFAULTS, { listAdvancedTypes: !0 }),
    (u.FE.PLUGINS.lists = function(d) {
      ...
      function r(e, t) {
        ...
        e.length &&
          (n =
            'rtl' == d.opts.direction || 'rtl' == u(e[0]).css('direction')
              ? 'margin-right'
              : 'margin-left');
        for (var o = 0; o < e.length; o++)
          if ('TD' != e[o].tagName && 'TH' != e[o].tagName && 'LI' != e[o].tagName) {
            ...
            var s = 0 < r ? '<' + t + ' style="' + n + ': ' + r + 'px;">' : '<' + t + '>',
              i = '</' + t + '>';
            for (l -= r; 0 < l / d.opts.indentMargin; )
              (s += '<' + t + '>'), (i += i), (l -= d.opts.indentMargin);
            a && e[o].tagName.toLowerCase() == a
              ? u(e[o]).replaceWith(
                  s + '<li' + d.node.attributes(e[o]) + '>' + u(e[o]).html() + '</li>' + i
                )
              : u(e[o]).wrap(s + '<li></li>' + i);
          }
        d.clean.lists();
      }
      function o(e) {
        ...
        for (t = 0; t < e.length; t++) {
          var r = u(e[t]),
            o = e[t].parentNode,
            l = r.attr('class');
          if ((r.before(g(o.tagName)), 'LI' == o.parentNode.tagName))
            r.before(g('LI')), r.after(c('LI'));
          else {
            var s = '';
            l && (s += ' class="' + l + '"');
            var i =
              'rtl' == d.opts.direction || 'rtl' == r.css('direction')
                ? 'margin-right'
                : 'margin-left';
            d.helpers.getPX(u(o).css(i)) &&
              0 <= (u(o).attr('style') || '').indexOf(i + ':') &&
              (s += ' style="' + i + ':' + d.helpers.getPX(u(o).css(i)) + 'px;"'),
              d.html.defaultTag() &&
                0 === r.find(d.html.blockTagsQuery()).length &&
                r.wrapInner('<' + d.html.defaultTag() + s + '></' + d.html.defaultTag() + '>'),
              d.node.isEmpty(r.get(0), !0) ||
                0 !== r.find(d.html.blockTagsQuery()).length ||
                r.append('<br>'),
              r.append(c('LI')),
              r.prepend(g('LI'));
        ...
        for (t = 0; t < a.length; t++) {
          var p = u(a[t]),
            f = p.html();
          (f = (f = f.replace(/<span class="fr-close-([a-z]*)"><\/span>/g, '</$1>')).replace(
            /<span class="fr-open-([a-z]*)"><\/span>/g,
            '<$1>'
          )),
            p.replaceWith(d.node.openTagString(p.get(0)) + f + d.node.closeTagString(p.get(0)));
        }
        d.$el.find('li:empty').remove(),
          d.$el.find('ul:empty, ol:empty').remove(),
          d.clean.lists(),
          d.html.wrap();
      }
      function l(e) {
        ...
        for (var t = 0; t < e.length; t++) {
          ...
          if (n) {
            ...
            if (a) {
              for (
                var r = u('<li>').prependTo(u(a)), o = d.node.contents(e[t])[0];
                o && !d.node.isList(o);
      ...
      function e(e) {
        if ('indent' == e || 'outdent' == e) {
          for (var t = !1, n = d.selection.blocks(), a = [], r = 0; r < n.length; r++)
            'LI' == n[r].tagName
              ? ((t = !0), a.push(n[r]))
              : 'LI' == n[r].parentNode.tagName && ((t = !0), a.push(n[r].parentNode));
          t && ('indent' == e ? l(a) : s(a));
        }
      }
      return {
            ...
            d.events.on(
              ...
              function(e) {
                if (e.which == u.FE.KEYCODE.TAB) {
                  for (var t = d.selection.blocks(), n = [], a = 0; a < t.length; a++)
                    'LI' == t[a].tagName
                      ? n.push(t[a])
                      : 'LI' == t[a].parentNode.tagName && n.push(t[a].parentNode);
                  if (
                    1 < n.length ||
                    (n.length && (d.selection.info(n[0]).atStart || d.node.isEmpty(n[0])))
                  )
                    return e.preventDefault(), e.stopPropagation(), e.shiftKey ? s(n) : l(n), !1;
                }
              },
              !0
            );
        ...
        format: function(e, t) {
          ...
          for (
            d.selection.save(),
              d.html.wrap(!0, !0, !0, !0),
              d.selection.restore(),
              a = d.selection.blocks(),
              n = 0;
            n < a.length;
            n++
          )
            'LI' != a[n].tagName && 'LI' == a[n].parentNode.tagName && (a[n] = a[n].parentNode);
          if (
            (d.selection.save(),
            (function(e, t) {
            ...
            for (a = d.selection.blocks(), n = 0; n < a.length; n++)
              'LI' != a[n].tagName && 'LI' == a[n].parentNode.tagName && (a[n] = a[n].parentNode);
            for (n = 0; n < a.length; n++)
              'LI' == a[n].tagName &&
                (u(a[n].parentNode).css('list-style-type', 'default' === t ? '' : t),
                0 === (u(a[n].parentNode).attr('style') || '').length &&
                  u(a[n].parentNode).removeAttr('style'));
          }
        },
