...
!(function(i) {
  ...
  T.extend(T.FE.POPUP_TEMPLATES, { 'file.insert': '[_BUTTONS_][_UPLOAD_LAYER_][_PROGRESS_BAR_]' }),
    T.extend(T.FE.DEFAULTS, {
      fileUpload: !0,
      fileUploadURL: null,
    ...
    (T.FE.PLUGINS.file = function(f) {
      ...
      function g() {
        ...
        e || (e = S()),
          ...
          n(f.language.translate('Uploading'), 0);
      }
      function o(e) {
        ...
        t &&
          ...
          e && (f.events.focus(), f.popups.hide('file.insert')));
      }
      function n(e, t) {
                ...
                try {
                  var t = T(e)
                    ...
                    i = T(e)
                      .find('Key')
                      .text();
                  return !1 === f.events.trigger('file.uploadedToS3', [t, i, e], !0)
                    ? (f.edit.on(), !1)
                    : t;
                } catch (r) {
                  return U(u, e), !1;
      ...
      function U(e, t) {
        f.edit.on(),
          ...
          f.events.trigger('file.error', [{ code: e, message: i[e] }, t]);
      }
      function w() {
        f.edit.on(), o(!0);
      }
      function a(e) {
        if (void 0 !== e && 0 < e.length) {
          ...
          var t,
            i = e[0];
          if (
            (null === f.opts.fileUploadURL || f.opts.fileUploadURL == p) &&
            !f.opts.fileUploadToS3
          )
            return (
              (s = i),
              ((l = new FileReader()).onload = function() {
                for (
                  var e = l.result, t = atob(l.result.split(',')[1]), i = [], r = 0;
                  r < t.length;
                  r++
                )
                  i.push(t.charCodeAt(r));
                (e = window.URL.createObjectURL(new Blob([new Uint8Array(i)], { type: s.type }))),
          ...
          if (
            f.opts.fileAllowedTypes.indexOf('*') < 0 &&
            f.opts.fileAllowedTypes.indexOf(i.type.replace(/file\//g, '')) < 0
          )
            return U(v), !1;
          if (
            (f.drag_support.formdata && (t = f.drag_support.formdata ? new FormData() : null), t)
          ) {
            var r;
            if (!1 !== f.opts.fileUploadToS3)
              for (r in (t.append(
                'key',
                f.opts.fileUploadToS3.keyStart + new Date().getTime() + '-' + (i.name || 'untitled')
              ),
              t.append('success_action_status', '201'),
              t.append('X-Requested-With', 'xhr'),
              t.append('Content-Type', i.type),
              f.opts.fileUploadToS3.params))
                f.opts.fileUploadToS3.params.hasOwnProperty(r) &&
                  t.append(r, f.opts.fileUploadToS3.params[r]);
            ...
            f.opts.fileUploadToS3 &&
              (o = f.opts.fileUploadToS3.uploadURL
                ? f.opts.fileUploadToS3.uploadURL
                : 'https://' +
                  f.opts.fileUploadToS3.region +
                  '.amazonaws.com/' +
                  f.opts.fileUploadToS3.bucket);
            var n = f.core.getXHR(o, f.opts.fileUploadMethod);
            (n.onload = function() {
              m.call(n, i.name);
            }),
              (n.onerror = b),
              (n.upload.onprogress = y),
              (n.onabort = w),
              g();
            var a = f.popups.get('file.insert');
            a &&
              a.off('abortUpload').on('abortUpload', function() {
      ...
      function S(e) {
        ...
        var t;
        f.opts.fileUpload ||
          f.opts.fileInsertButtons.splice(f.opts.fileInsertButtons.indexOf('fileUpload'), 1),
          (t =
            '<div class="fr-buttons">' + f.button.buildList(f.opts.fileInsertButtons) + '</div>');
        var i = '';
        f.opts.fileUpload &&
          (i =
            ...
            f.id +
            '"><strong>' +
            f.language.translate('Drop file') +
            '</strong><br>(' +
            f.language.translate('or click') +
            ')<div class="fr-form"><input type="file" name="' +
            f.opts.fileUploadParam +
            '" accept="' +
            (0 <= f.opts.fileAllowedTypes.indexOf('*') ? '/' : '') +
            f.opts.fileAllowedTypes.join(', ').toLowerCase() +
        ...
        return (
          ...
          f.events.$on(
            ...
            function() {
              return T(this).addClass('fr-drop'), !1;
            },
            !0
          ),
          f.events.$on(
            ...
            '.fr-file-upload-layer',
            function() {
              return T(this).removeClass('fr-drop'), !1;
            },
            !0
          ),
          f.events.$on(
            ...
            '.fr-file-upload-layer',
            function(e) {
              e.preventDefault(), e.stopPropagation(), T(this).removeClass('fr-drop');
          ...
          f.helpers.isIOS() &&
            f.events.$on(r, 'touchstart', '.fr-file-upload-layer input[type="file"]', function() {
          ...
          if (i && 'undefined' != typeof i.type) {
            if (i.type.indexOf('image') < 0) {
              ...
              f.markers.remove(),
                ...
                f.$el.find('.fr-marker').replaceWith(T.FE.MARKERS),
                f.popups.hideAll();
      ...
      return (
        (i[1] = 'File cannot be loaded from the passed link.'),
        ...
        (i[7] = 'Files can be uploaded only to same domain in IE 8 and IE 9.'),
        {
          _init: function() {
            f.events.on('drop', t),
              f.events.$on(f.$win, 'keydown', function(e) {
                ...
                i && t == T.FE.KEYCODE.ESC && i.trigger('abortUpload');
              }),
              ...
              f.$wp && (C(), f.events.on('contentChanged', C)),
              S(!0);
          },
          showInsertPopup: function() {
            ...
            if ((t || (t = S()), o(), !t.hasClass('fr-active')))
              if (
                (f.popups.refresh('file.insert'),
                f.popups.setContainer('file.insert', f.$tb),
                e.is(':visible'))
              ) {
    ...
    T.FE.DefineIcon('insertFile', { NAME: 'file-o', FA5NAME: 'file' }),
    T.FE.RegisterCommand('insertFile', {
      title: 'Upload File',
      undo: !1,
      focus: !0,
    ...
    T.FE.DefineIcon('fileBack', { NAME: 'arrow-left' }),
    T.FE.RegisterCommand('fileBack', {
      title: 'Back',
      undo: !1,
      focus: !1,
      back: !0,
    ...
    T.FE.RegisterCommand('fileDismissError', {
      title: 'OK',
      callback: function() {
